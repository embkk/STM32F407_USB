...
*
*  Parameters
*    sSrvAddr : Server address as a string.
*    SrvPort  : Server port as an unsigned integer.
*
*  Return value
*    != NULL: The connected socket cast to IP_MQTT_CLIENT_SOCKET.
*    == NULL: Connect failed.
*/
typedef void* (IP_MQTT_CLIENT_CONNECT)(const char* sSrvAddr, unsigned SrvPort);
...
*
*  Parameters
*    pSocket : Pointer to the socket to disconnect.
*/
typedef void (IP_MQTT_CLIENT_DISCONNECT)(void* pSocket);

/*********************************************************************
*
*       IP_MQTT_CLIENT_RECEIVE
*
*  Description
*    Receives data from the specified socket.
*
*  Parameters
*    pSocket : Pointer to the socket to receive data from.
...
*    == -1: Error.
*/
typedef int (IP_MQTT_CLIENT_RECEIVE)(void* pSocket, char* pData, int Len);
...
*
*  Parameters
*    pSocket : Pointer to the socket to send data to.
*    pData   : Pointer to the data buffer.
*    Len     : Length of the data buffer.
*
*  Return value
...
*    == -1: Error.
*/
typedef int (IP_MQTT_CLIENT_SEND)(void* pSocket, const char* pData, int Len);
...
*/
typedef struct {
  IP_MQTT_CLIENT_CONNECT *    pfConnect;    // Pointer to the connect function (for example, connect()).
  IP_MQTT_CLIENT_DISCONNECT * pfDisconnect; // Pointer to the disconnect function (for example, closesocket()).
  IP_MQTT_CLIENT_RECEIVE *    pfReceive;    // Pointer to a callback function (for example, recv()).
  IP_MQTT_CLIENT_SEND *       pfSend;       // Pointer to a callback function (for example, send()).
} IP_MQTT_CLIENT_TRANSPORT_API;
...
typedef struct { // NOLINT(clang-analyzer-optin.performance.Padding) (Public struct)
  ...
  U8                      CheckUserTimeout; // (Optional) Set this to 1 to use the UserTimeout. Otherwise UserTimeout is ignored.
  int                     UserTimeout;      // (Optional) This value can be set by the user and evaluated inside the IP_MQTT_CLIENT_CHECK_TIMEOUT_CB callback.
...
/*********************************************************************
*
*       IP_MQTT_CLIENT_GEN_RANDOM
*
*  Description
*    Generates a random unsigned short value.
*
*  Return value
*    Random unsigned short value.
*/
typedef U16 (IP_MQTT_CLIENT_GEN_RANDOM)(void);

/*********************************************************************
*
*       IP_MQTT_CLIENT_ALLOC
*
*  Description
...
*
*  Return value
*    != NULL: Pointer to the allocated memory.
*    == NULL: Alloc failed.
*/
typedef void* (IP_MQTT_CLIENT_ALLOC)(U32 NumBytesReq);
...
*/
typedef void (IP_MQTT_CLIENT_FREE)(void* p);
...
*/
typedef void (IP_MQTT_CLIENT_LOCK)(void);

/*********************************************************************
*
*       IP_MQTT_CLIENT_UNLOCK
*
*  Description
*    Releases a previously acquired lock.
*/
typedef void (IP_MQTT_CLIENT_UNLOCK)(void);

/*********************************************************************
*
*       IP_MQTT_CLIENT_RECV_MESSAGE
*
*  Description
*    Deprecated, use IP_MQTT_CLIENT_RECV_MESSAGE_EX
*/
typedef int (IP_MQTT_CLIENT_RECV_MESSAGE)(void* pMQTTClient, void* pPublish, int NumBytesRem);

/*********************************************************************
*
*       IP_MQTT_CLIENT_ON_MESSAGE_CONFIRM
*
*  Description
*    Deprecated, use IP_MQTT_CLIENT_ON_MESSAGE_CONFIRM_EX
*/
typedef int (IP_MQTT_CLIENT_ON_MESSAGE_CONFIRM)(void* pMQTTClient, U8 Type, U16 PacketId);

/*********************************************************************
*
*       IP_MQTT_CLIENT_HANDLE_ERROR
*
*  Description
*    Callback which is called in case of errors on the transport layer.
*
*  Parameters
*    pMQTTClient : Valid pointer to an MQTT client instance.
*
*  Return value
*    Not used in this version.
*    Should be set to 0.
*/
typedef int (IP_MQTT_CLIENT_HANDLE_ERROR)(void* pMQTTClient);

/*********************************************************************
*
*       IP_MQTT_CLIENT_HANDLE_DISCONNECT
*
*  Description
...
*  Parameters
*    pMQTTClient : Valid pointer to an MQTT client instance.
*    ReasonCode  : MQTT reason code for disconnection.
*/
typedef void (IP_MQTT_CLIENT_HANDLE_DISCONNECT)(void* pMQTTClient, U8 ReasonCode);

/*********************************************************************
*
*       IP_MQTT_CLIENT_ON_MESSAGE_CONFIRM_EX
*
*  Description
*    Callback which is called when all QoS related messages are processed.
*
*  Parameters
*    pMQTTClient : Valid pointer to an MQTT client instance.
*    Type        : Type of the message.
*    PacketId    : ID of the packet.
*    ReasonCode  : MQTT reason code for confirmation.
*
*  Return value
*    == 1: Confirmation was handled. IP_MQTT_CLIENT_FREE will be called.
*    != 1: Confirmation not handled, message will not be freed.
*
...
*/
typedef int (IP_MQTT_CLIENT_ON_MESSAGE_CONFIRM_EX)(void* pMQTTClient, U8 Type, U16 PacketId, U8 ReasonCode);

/*********************************************************************
*
*       IP_MQTT_CLIENT_RECV_MESSAGE_EX
*
*  Description
*    Receives a MQTT PUBLISH message.
*
*  Parameters
*    pMQTTClient : Valid pointer to an MQTT client instance.
*    pPublish    : Valid pointer to the publish structure.
*    NumBytesRem : Number of remaining bytes.
*    pReasonCode : [OUT] (QoS 1 and 2 only) pointer to a U8 where
*                  the callback must store the MQTT reason code
...
*    <  0: Error
*/
typedef int (IP_MQTT_CLIENT_RECV_MESSAGE_EX)(void* pMQTTClient, void* pPublish, int NumBytesRem, U8 * pReasonCode);

/*********************************************************************
*
*       IP_MQTT_CLIENT_ON_PROPERTY
*
*  Description
*    Handles properties in non-PUBLISH MQTT messages.
*
*  Parameters
*    pMQTTClient : Valid pointer to an MQTT client instance.
*    PacketId    : ID of the packet.
*    PacketType  : Type of the packet.
*    pProp       : Pointer to the property.
*/
typedef void (IP_MQTT_CLIENT_ON_PROPERTY)(void* pMQTTClient, U16 PacketId, U8 PacketType, IP_MQTT_PROPERTY * pProp);

/*********************************************************************
*
*       IP_MQTT_CLIENT_CHECK_TIMEOUT_CB
*
*  Description
*    Allows the application to time out messages which are not
*    receiving a reply from the server.
*
*    This callback is called in the context of IP_MQTT_CLIENT_CheckMessageTimeouts().
*    The function IP_MQTT_CLIENT_CheckMessageTimeouts() must be called by the application
*    periodically in order for this callback to work.
*
*  Parameters
*    pClient : Valid pointer to a IP_MQTT_CLIENT_CONTEXT instance.
*    pMessage: Valid pointer to a IP_MQTT_CLIENT_MESSAGE structure for
*              a MQTT message which has not received a reply yet.
*              (QoS 1 and QoS 2 only)
*
*  Return value
*    ==  0: pMessage not timed out yet.
*    ==  1: pMessage timed out. The emMQTT code will call
*           the IP_MQTT_CLIENT_APP_API->pfFree callback.
*
*  Additional information
*    Only messages with IP_MQTT_CLIENT_MESSAGE->CheckUserTimeout set to 1 are checked.
*    Before sending a message the application should set
*    the IP_MQTT_CLIENT_MESSAGE->UserTimeout to a value which
*    the application can compare against.
*    Normally it will be the current time + the timeout value, but it
*    can also be as simple as a counter variable.
*/
typedef int (IP_MQTT_CLIENT_CHECK_TIMEOUT_CB) (void* pMQTTClient, IP_MQTT_CLIENT_MESSAGE* pMessage);
...
*/
typedef struct {
  IP_MQTT_CLIENT_GEN_RANDOM *           pfGenRandom;           // Pointer to a function which returns a random unsigned short value.
  IP_MQTT_CLIENT_ALLOC *                pfAlloc;               // Pointer to a alloc function.
  IP_MQTT_CLIENT_FREE *                 pfFree;                // Pointer to a free function.
  IP_MQTT_CLIENT_LOCK *                 pfLock;                // Pointer to a function which acquires a lock.
  IP_MQTT_CLIENT_UNLOCK *               pfUnlock;              // Pointer to a function which releases a previously acquired lock.
  IP_MQTT_CLIENT_RECV_MESSAGE *         pfRecvMessage;         // Deprecated, use IP_MQTT_CLIENT_RECV_MESSAGE_EX
  IP_MQTT_CLIENT_ON_MESSAGE_CONFIRM *   pfOnMessageConfirm;    // Deprecated, use IP_MQTT_CLIENT_ON_MESSAGE_CONFIRM_EX
  IP_MQTT_CLIENT_HANDLE_ERROR *         pfHandleError;         // Pointer to a callback which is called in case of an error.
  IP_MQTT_CLIENT_HANDLE_DISCONNECT *    pfHandleDisconnect;    // [MQTT 5 only] Pointer to an optional callback when the server sends a Disconnect Request.
  IP_MQTT_CLIENT_ON_MESSAGE_CONFIRM_EX *pfOnMessageConfirmEx;  // Pointer to a callback when all QoS related messages are processed.
  IP_MQTT_CLIENT_RECV_MESSAGE_EX *      pfRecvMessageEx;       // Pointer to a callback when a PUBLISH message is received.
  IP_MQTT_CLIENT_ON_PROPERTY *          pfOnProperty;          // [MQTT 5 only] Pointer to an optional callback for non-PUBLISH message properties.
  IP_MQTT_CLIENT_CHECK_TIMEOUT_CB *     pfOnCheckTimeout;      // Pointer to a callback which checks if an outgoing message timed out.
} IP_MQTT_CLIENT_APP_API;
...
int  IP_MQTT_CLIENT_CheckMessageTimeouts(IP_MQTT_CLIENT_CONTEXT* pClient);
