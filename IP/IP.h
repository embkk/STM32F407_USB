...
#ifndef IP_H_       // Avoid multiple inclusion.
#define IP_H_
...
#include "IP_Socket.h"
#include "BSP_IP.h"            // IP_BSP_ types have been outsourced to eliminate BSP_IP.c requiring IP.h .
...
#define IP_VERSION  36204u  // Format: Mmmrr. Example: 35801 is 3.58.1
...
#define IP_MTYPE_IPV6         (IP_MTYPE_AUTOIP)    // Temporary message type. Reuse IP_MTYPE_AUTOIP, since it is normally not used.
#define IP_MTYPE_VLAN         (IP_MTYPE_DHCP_EXT)  // Temporary message type. Reuse IP_MTYPE_DHCP_EXT, since it is normally not used.
...
#define     IP_ERR_NOMEM           (-29)   //lint !e9046 System error. The lint note is caused by IP_ERR_NO_MEM, but we have to keep it for backward compatibility
...
#define     IP_ERR_USER_ABORT      (-38)

#define     IP_ERR_TRIAL_LIMIT     (-128)  // Trial library limit reached

#define     IP_ERR_NO_MEM IP_ERR_NOMEM     //lint !e9046 "IP_ERR_NO_MEM" is obsolete, redefined to "IP_ERR_NOMEM" for backwards compatibility.
...
*  usage in IP_ConfigIO.c to be able to use embOSView via UDP for
*  log messages.
*/
typedef void (IP_ON_EXIT_FUNC)(void);

typedef struct IP_ON_EXIT_HOOK_STRUCT IP_ON_EXIT_HOOK;
struct IP_ON_EXIT_HOOK_STRUCT {
  IP_ON_EXIT_HOOK* pNext;
  IP_ON_EXIT_FUNC* pf;
};

void IP_AddOnExitHandler(IP_ON_EXIT_HOOK* pHook, IP_ON_EXIT_FUNC* pf);
#define IP_ON_EXIT_CB  IP_ON_EXIT_HOOK
#endif
...
*/
typedef struct {
  U32 Version;  // The version of the IP_OS layer does not necessarily
                // reflect the latest version of the stack itself. It
                // uses the IP_VERSION of the stack in which an updated
                // IP_OS layer has been made available with an important
                // change to at least one of the available IP_OS layers.
                // This allows a debug build to check for an outdated
                // IP_OS layer to be used that might miss an important
                // change or bugfix that has been made available since.
} IP_OS_API;
...
//
// Wait and signal for the optional DTask (driver) task
//
...
typedef   U32 ip_addr; //lint !e9046 Ignore Typographical ambiguity note, we are keeping this for backward compatibility.
typedef   U32 IP_ADDR; //lint !e9046 Ignore Typographical ambiguity note, we are keeping this for backward compatibility.
//
// Structure for TCP segment header in IPv4
//
typedef struct {
  U16 PortSrc;      // Source port number
  U16 PortDest;     // Destination port number
  U32 SeqNo;        // Sequence number
  U32 AckNo;        // Acknowledgment number
  U8  HeaderLen;    // Header length
  U8  Flags;        // TCP flags
  U16 WindowSize;   // Window size
  U16 CheckSum;     // Checksum
} IP_TCP4;
//
// Structure for Ethernet and IPv4 header with TCP segment
//
typedef struct {
  U16 Dummy;               // Dummy field
  U8  aMACDest[6];         // Destination MAC address
  U8  aMACSrc [6];         // Source MAC address
  U16 Type;                // Ethernet type
  // IP
  U8  VersionHeaderLen;    // Version and header length
  U8  Service;             // Type of service
  U16 Len;                 // Total length
  U16 PacketId;            // Identification
  U16 FragmentOff;         // Fragment offset
  U8  TTL;                 // Time to live
  U8  Proto;               // Protocol
  U16 HeaderChecksum;      // Header checksum
  U32 IPSrc;               // Source IP address
  U32 IPDest;              // Destination IP address
  IP_TCP4 Tcp;             // TCP segment
} IP_ETH_IP4_TCP;
//
// Structure for TTL configuration
//
typedef struct {
  U8  DefaultTTL;   // Default TTL value
  U8  LocalMcTTL;   // TTL for local multicast
  U8  GlobalMcTTL;  // TTL for global multicast
} IP_TTL_CONFIG;
...
struct IP_PACKET_STRUCT {
  ...
  U64               OSTimestamp_ns;      // Time stamp (nanoseconds) based on OS time.
#endif
#if IP_SUPPORT_PTP
...
typedef void (*IP_WIFI_pfOnSignalChange)      (unsigned IFaceId, const IP_WIFI_SIGNAL_INFO* pInfo);
...
#define IP_WIFI_SECURITY_OPEN                       (0u)
#define IP_WIFI_SECURITY_WEP_OPEN                   (1u)
#define IP_WIFI_SECURITY_WEP_SHARED                 (2u)
#define IP_WIFI_SECURITY_WPA_TKIP                   (3u)
#define IP_WIFI_SECURITY_WPA_AES                    (4u)
#define IP_WIFI_SECURITY_WPA2_AES                   (5u)
#define IP_WIFI_SECURITY_WPA_WPA2_MIXED             (6u)                              //lint !e621 Ignore identifier clash for 31 unique chars in name.
#define IP_WIFI_SECURITY_WPA_ENTERPRISE             (7u)
...
struct IP_HOOK_ON_DTASK_EXEC_DONE_STRUCT {
  ...
  unsigned                    IFaceId;  // Zero-based interface index for which the callback will be executed.
      ...
      void     IP_DTASK_SetTimeout              (unsigned IFaceId, unsigned Timeout);
      ...
      void     IP_WIFI_RemoveAssociateChangeHook(IP_HOOK_ON_WIFI_ASSOCIATE_CHANGE* pHook);
      void     IP_WIFI_AddClientNotificationHook(IP_HOOK_ON_WIFI_CLIENT_NOTIFICATION* pHook, IP_WIFI_pfOnClientNotification pf);
      void     IP_WIFI_RemoveClientNotificationHook(IP_HOOK_ON_WIFI_CLIENT_NOTIFICATION* pHook);
      ...
      void     IP_WIFI_RemoveSignalChangeHook   (IP_HOOK_ON_WIFI_SIGNAL_CHANGE* pHook);
...
// Compatibility macros
//
#define IP_DTASK_ConfigTimeout  IP_DTASK_SetTimeout
...
#define IP_NI_CAPS_ALL_CHKSUM  (IP_NI_CAPS_WRITE_IP_CHKSUM      | \
...
typedef struct {
  ...
  void     (*pfWrite)(IP_PHY_CONTEXT* pContext, unsigned RegIndex, unsigned v);
} IP_PHY_ACCESS;
...
#define IP_PHY_MODE_NO_AUTONEG  ((U16)1u << 11)  // Pseudo bit. Bit is not consecutive to the rest as other registers and bits are used.
...
/*********************************************************************
*
*       IP_PHY_DRIVER_ATTACH_CONTEXT_FUNC
*
*  Function description
*    Driver specific callback to attach a driver specific context to
*    the PHY context for managing PHY driver specific settings that
*    are typically configured via the control function.
*
*  Parameters
*    IFaceId   : Zero-based interface index.
*    pContextEx: PHY driver context.
*
*  Return value
*    == 0: O.K.
*    != 0: Error.
*/
typedef int (IP_PHY_DRIVER_ATTACH_CONTEXT_FUNC)(unsigned IFaceId, IP_PHY_CONTEXT_EX* pContext);

/*********************************************************************
*
*       IP_PHY_DRIVER_INIT_FUNC
*
*  Function description
*    Driver specific callback to initialize PHY driver.
*
*  Parameters
*    IFaceId   : Zero-based interface index.
*    pContextEx: PHY driver context.
...
*    != 0: Error.
*/
typedef int (IP_PHY_DRIVER_INIT_FUNC)(unsigned IFaceId, IP_PHY_CONTEXT_EX* pContext);

/*********************************************************************
*
*       IP_PHY_DRIVER_GET_LINK_STATE_FUNC
*
*  Function description
*    Driver specific callback to retrieve the current link state of the PHY.
*
*  Parameters
*    IFaceId   : Zero-based interface index.
*    pContextEx: PHY driver context.
*    pDuplex   : Pointer where to store the duplex value.
*    pSpeed    : Pointer where to store the speed value.
*
*  Return value
*    == 0: O.K.
*    != 0: Error.
*/
typedef int (IP_PHY_DRIVER_GET_LINK_STATE_FUNC)(unsigned IFaceId, IP_PHY_CONTEXT_EX* pContext, U32* pDuplex, U32* pSpeed);

/*********************************************************************
*
*       IP_PHY_DRIVER_CONTROL_FUNC
*
*  Function description
*    Driver specific callback to execute various jobs based on commands.
*
*  Parameters
*    IFaceId   : Zero-based interface index.
*    pContextEx: PHY driver context.
*    Cmd       : Command to execute.
*    p         : Optional parameters based on Cmd.
*
*  Return value
*    == -1: Command not supported.
*    ==  0: Command supported, O.K.
*    >   0: Command supported, Error.
*/
typedef int (IP_PHY_DRIVER_CONTROL_FUNC)(unsigned IFaceId, IP_PHY_CONTEXT_EX* pContext, unsigned Cmd, void* p);

/*********************************************************************
*
*       IP_PHY_DRIVER_READ_REG_FUNC
*
*  Function description
*    Driver specific callback to read a PHY register.
*
*  Parameters
*    IFaceId   : Zero-based interface index.
*    pContextEx: PHY driver context.
...
*    != 0: Error.
*/
typedef int (IP_PHY_DRIVER_READ_REG_FUNC)(unsigned IFaceId, IP_PHY_CONTEXT_EX* pContextEx, unsigned RegIndex, unsigned* pData);

/*********************************************************************
*
*       IP_PHY_DRIVER_WRITE_REG_FUNC
*
*  Function description
*    Driver specific callback to write a PHY register.
*
*  Parameters
*    IFaceId   : Zero-based interface index.
*    pContextEx: PHY driver context.
...
*    != 0: Error.
*/
typedef int (IP_PHY_DRIVER_WRITE_REG_FUNC)(unsigned IFaceId, IP_PHY_CONTEXT_EX* pContextEx, unsigned RegIndex, unsigned Data);

typedef struct IP_PHY_HW_DRIVER {
  IP_PHY_DRIVER_ATTACH_CONTEXT_FUNC* pfAttachContext;
  IP_PHY_DRIVER_INIT_FUNC*           pfInit;
  IP_PHY_DRIVER_GET_LINK_STATE_FUNC* pfGetLinkState;
  IP_PHY_DRIVER_CONTROL_FUNC*        pfControl;
  IP_PHY_DRIVER_READ_REG_FUNC*       pfReadReg;
  IP_PHY_DRIVER_WRITE_REG_FUNC*      pfWriteReg;
} IP_PHY_HW_DRIVER;
...
typedef enum {
  IP_DRIVER_INTERFACE_INIT_ERROR = 0,
  IP_DRIVER_INTERFACE_PHY_ERROR  = 1
} IP_DRIVER_INTERFACE_ERROR;
...
*
*  Parameters
*   init     : Gives information about which init failed.
*              * init = IP_DRIVER_INTERFACE_INIT_ERROR: Error during driver init.
*              * init = IP_DRIVER_INTERFACE_PHY_ERROR: Error during PHY Init.
*   IFaceId  : Number of interface that failed.
*   ErrCode  : Error Code.
*/
typedef void (IP_HOOK_ON_IF_ERROR)(IP_DRIVER_INTERFACE_ERROR init, int IFaceId, int Errcode);
...
int  IP_PHY_ReadReg               (unsigned IFaceId, unsigned RegIndex, unsigned* pData);
void IP_PHY_ReInit                (unsigned IFaceId);
void IP_PHY_SetWdTimeout          (int ShiftCnt);
int  IP_PHY_WriteReg              (unsigned IFaceId, unsigned RegIndex, unsigned Data);
...
typedef void (IP_TX_HOOK)      (IP_PACKET *pPacket);
#endif
...
#define IP_ConfTCPSpace  IP_ConfigTCPSpace  // Compatibility macro for old API
      ...
      void       IP_RemoveLinkChangeHook            (IP_HOOK_ON_LINK_CHANGE* pHook);
...
const char *     IP_GetRawPacketInfo                (const IP_PACKET *pPacket, U16 *pNumBytes);
      ...
      int        IP_Init                            (void);
      int        IP_IPV4_ParseIPv4Addr              (const char* sHost, IP_ADDR* pIPv4Addr);
      unsigned   IP_IsAllZero                       (const U8* p, unsigned NumBytes);
      unsigned   IP_IsExpired                       (U32 Time);
      unsigned   IP_IsInitialized                   (void);
#ifdef __clang_analyzer__
      void       IP_Panic                           (const char *sError) __attribute__((analyzer_noreturn));
#else
      ...
      int        IP_SendPacket                      (unsigned IFace, void* pData, unsigned NumBytes); //lint !e9046 Ignore typographical ambiguity with 'IP__SendPacket'
...
#if IP_SUPPORT_ON_PACKET_TX_CB
      void       IP_SetTxHook                       (IP_TX_HOOK *pfTxHook);
#endif
      int        IP_SetSupportedDuplexModes         (unsigned IFaceId, unsigned DuplexMode);
...
*
*  Description
*    Structure which stores information about the VLAN.
*/
typedef struct IP_VLAN_INIT_DATA {
  U8 Use8100EtherType;  // By default the interface will use EtherType of 0x88A8 (TPID) for the STAG, when this is set to 1 0x8100 is used instead (the CTAG always uses 0x8100).
} IP_VLAN_INIT_DATA;

int IP_VLAN_AddInterface(unsigned HWIFace, U16 VLANId);
int IP_VLAN_Add8021adInterface(unsigned HWIFace, U16 VLAN_STAG, U16 VLAN_CTAG, const IP_VLAN_INIT_DATA * pInitData);
...
//
typedef struct {
  const char*               sName;        // No need to have a A record for the hostname. The IP address of the interface is used in this case.
  IP_ADDR                   IPAddr;
} IP_DNS_SERVER_A;

#if IP_SUPPORT_IPV6
typedef struct {
  const char*               sName;        // No need to have a AAAA record for the hostname. The IP address of the interface is used in this case.
  U8                        aIPAddrV6[IPV6_ADDR_LEN];
} IP_DNS_SERVER_AAAA;
#endif

typedef struct {
  const char*               sName;        // Either a real name _service._udp.local or a formated IP address 01.00.168.192.in-addr.arpa
  const char*               sDomainName;  // If NULL, hostname will be used.
} IP_DNS_SERVER_PTR;
...
typedef struct {
  ...
  U16                       Weight;
  U16                       Port;
  const char*               sTarget;      // If NULL, hostname will be used.
} IP_DNS_SERVER_SRV;

typedef struct {
  const char*               sName;        // If NULL, hostname will be used.
  const char*               sTXT;
} IP_DNS_SERVER_TXT;
...
#define  DHCPC_STATE_LEASE_EXPIRED  13u  // A previously acquired configuration timed out as the server did not respond.
...
int      IP_DHCPC_Release                   (int IFaceId);
...
typedef enum {
  ...
  IP_ACD_STATE_ACTIVE_WAIT_BEFORE_BG_PROBES,  // Optional state that allows to utilize a delay before starting with background probing.
  IP_ACD_STATE_ACTIVE_BG_PERIOD_UPDATE,       // Transparent state that is used internally when modifying the background probing delay via API.
...
typedef struct {
  ...
  unsigned volatile WaitTime;  // Wait time before the next state that is used (does not have to obey min./max. suggestion).
...
typedef struct {
  ...
  IP_ACD_STATE               OldState;              // Previous state. Might be the same as new state depending on actions executed in callbacks. If filtering is needed, this needs to be implemented in the application.
                                                    // When counting events like how many announcements have been sent, the OldState should be used for filtering as this reports the event handled immediately before or
                                                    // after reporting the state (change).
...
typedef struct {
  ...
  IP_ACD_STATE InitState;              // Initial state for the ACD state machine upon activating ACD. The following states are supported:
                                       //   * IP_ACD_STATE_DISABLED
                                       //     Default behavior starting ACD listening for potential conflicts at the beginning.
                                       //   * IP_ACD_STATE_ANNOUNCE_SEND_GARP
                                       //     Skips the initial listening phase and starts by directly sending the first the first announcement.
                                       //     This can be used to implement EtherNet/IP "QuickConnect" behavior.
} IP_ACD_EX_CONFIG;
...
void IP_ACD_UpdateBackgroundPeriod(unsigned IFaceId, unsigned BackgroundPeriod);
