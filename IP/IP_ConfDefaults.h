...
//
  ...
  #define IP_OS_WAIT_RX_EVENT(ms)         IP_OS_WaitRxEventTimed(ms)
#endif
...
#endif
//
// Can be set to 0 to disable statistics for extremely small release builds
//
#ifndef   IP_INCLUDE_STAT
  #define IP_INCLUDE_STAT (IP_DEBUG > 0)
#endif
...
//
// Min. delay for retransmit. Real delay is computed,
// this minimum applies only if computed delay is shorter.
// Min should be > 200, since 200 ms is a typ. value for delayed ACKs
//
//
#ifndef   IP_TCP_RETRANS_MIN
  #define IP_TCP_RETRANS_MIN              210
#endif
//
// Max. delay for retransmit. Real delay is computed,
// this maximum applies only if computed delay is longer.
//
#ifndef   IP_TCP_RETRANS_MAX
  #define IP_TCP_RETRANS_MAX              3000
#endif
//
// Number of retransmits before the connection gets dropped.
//
#ifndef   IP_TCP_RETRANS_NUM
  #define IP_TCP_RETRANS_NUM              6
#endif

//
// TCP keep-alive defaults
//
// Initial connect keep alive [ms]
//
#ifndef   IP_TCP_KEEPALIVE_INIT
  #define IP_TCP_KEEPALIVE_INIT           10000
#endif
//
// Default time before probing [ms]
//
#ifndef   IP_TCP_KEEPALIVE_IDLE
  #define IP_TCP_KEEPALIVE_IDLE           10000
#endif
//
// Default probe interval [ms]
//
#ifndef   IP_TCP_KEEPALIVE_PERIOD
  #define IP_TCP_KEEPALIVE_PERIOD         10000
#endif
//
// Max probes before drop
//
#ifndef   IP_TCP_KEEPALIVE_MAX_REPS
  #define IP_TCP_KEEPALIVE_MAX_REPS       8
#endif
//
// Max segment lifetime. Used primarily for the TCP TIME_WAIT state. Large value wastes a lot of time!
//
#ifndef   IP_TCP_MSL
  #define IP_TCP_MSL                      100
#endif
...
#endif
//
// Socket ID limit on which the list wraps back to the lowest ID (typically 1)
// when trying to allocate a socket. The limit ID itself is not used.
//
#ifndef   IP_SOCKET_ID_LIMIT
  #define IP_SOCKET_ID_LIMIT              0xFFFF
#endif
...
#endif
//
// Do we support RFC-1323 TCP timestamp feature to compute RTT ?
//
#ifndef   IP_TCP_SUPPORT_TIMESTAMP
  #define IP_TCP_SUPPORT_TIMESTAMP        1
#endif
//
// Adds the ACK flag to reset packets. MS Windows hosts send RST packets always with ACK flag and proper acknowledgement number.
//
#ifndef   IP_TCP_ADD_ACK_TO_RST_PACKETS
  #define IP_TCP_ADD_ACK_TO_RST_PACKETS   0
#endif
...
#endif
//
// Fulfill the UPnP spec (more messages). This has been tested under windows and does not seem to be needed.
//
#ifndef   IP_UPNP_FULFIL_SPEC
  #define IP_UPNP_FULFIL_SPEC             0
#endif
//
// Limit the number of UPnP requests that we can handle at the same time
// (especially limits bytes allocated for required contexts).
//
#ifndef   IP_UPNP_NUM_REQUESTS
  #define IP_UPNP_NUM_REQUESTS            5u
#endif
...
#endif
//
// Multicast TTL (basically in seconds but more often counted in router hops)
// for local networks inside the ranges 224.0.0.x and 239.x.x.x .
//
#ifndef   IP_LOCAL_MC_TTL_TIME
  #define IP_LOCAL_MC_TTL_TIME            1
#endif
//
// Multicast TTL (basically in seconds but more often counted in router hops)
// for local networks outside the ranges 224.0.0.x and 239.x.x.x .
//
#ifndef   IP_GLOBAL_MC_TTL_TIME
  #define IP_GLOBAL_MC_TTL_TIME           64
#endif
//
// TTL (basically in seconds but more often counted in router hops)
// for non-multicast packets.
//
#ifndef   IP_TTL_TIME
  #define IP_TTL_TIME                     64
#endif
...
#ifndef IP_PANIC
  #if   IP_DEBUG != 0 || defined(__clang_analyzer__)
    #define IP_PANIC(s)                   IP_Panic(s)
  #else
    #ifdef _lint
      //
      // Avoid false lint messages due to IP_PANIC not being declared in release configuration
      //lint -emacro({9036}, IP_PANIC)
      #define IP_PANIC(s)  do {} while(0)
    #else
      #define IP_PANIC(s)
    #endif
  #endif
...
#endif
//
 // Maximum number of interfaces to support at one time
//
#ifndef   IP_MAX_IFACES
  #define IP_MAX_IFACES                   1
#endif
//
// Sync timers with equal periods to execute together.
//
#ifndef   IP_TIMERS_USE_PERIOD_SYNC
  #define IP_TIMERS_USE_PERIOD_SYNC       1
#endif
//
// Maximum period [ms] that will be synced to each other.
//
#ifndef   IP_TIMERS_MAX_PERIOD_SYNC
  #define IP_TIMERS_MAX_PERIOD_SYNC       1000u
#endif

#ifndef   IP_TCP_DELAY_ACK_DEFAULT
  #define IP_TCP_DELAY_ACK_DEFAULT        200     // In ms
#endif
//
// Enables delayed ACKs. Old define, kept for compatibility, use IP_SUPPORT_TCP_DELAYED_ACK in code.
//
#ifndef   DO_DELAY_ACKS
  #define DO_DELAY_ACKS                   1
#endif
...
#endif
//
// IP_DeInit() can be used to de-initialize the stack
// Having this at 0 allows to same some ROM.
//
#ifndef   IP_ALLOW_DEINIT
  #define IP_ALLOW_DEINIT                 0
#endif
...
#ifndef   IP_SNTP_SERVER_TIMEOUT
  #define IP_SNTP_SERVER_TIMEOUT          10000u
#endif
...
#endif
//
// Create a short time ARP entry on Rx to avoid sending ARPs if we could already know the destination
//
#ifndef   IP_ARP_SNIFF_ON_RX
  #define IP_ARP_SNIFF_ON_RX              1
#endif
//
// Link probes before a link up is used by the stack. 1 means direct, 0 disables the code.
//
#ifndef   IP_NUM_LINK_UP_PROBES
  #define IP_NUM_LINK_UP_PROBES           1
#endif
//
// Link probes before a link down is used by the stack. 1 means direct, 0 disables the code.
//
#ifndef   IP_NUM_LINK_DOWN_PROBES
  #define IP_NUM_LINK_DOWN_PROBES         1
#endif
//
// Support adding a callback to be executed once a specific packet gets freed.
//
#ifndef   IP_SUPPORT_ON_PACKET_FREE_CB
  #define IP_SUPPORT_ON_PACKET_FREE_CB    0
#endif
//
// Support adding a callback to be executed once a specific packet gets transmitted.
//
#ifndef IP_SUPPORT_ON_PACKET_TX_CB
  #define IP_SUPPORT_ON_PACKET_TX_CB      0
#endif
//
// Tx FIFO stuck check is done from 1s interface (link check) timer right now.
// For the moment this is not the definitive timeout but more like a placeholder to define a timeout
// at all and might be used with its real value if implementing a dedicated timer in the future.
//
#ifndef   IP_TX_FIFO_TIMEOUT
  #define IP_TX_FIFO_TIMEOUT              100u
#endif
//
...
// When enabling this please also call IP_FRAGMENT_Enable() in your IP_X_Config().
//
...
//
  ...
  #define IP_FRAGMENT_CHECKS_PER_SEC      10
#endif
//
// Number of allowed fragments. 0xFF means no limit.
//
#ifndef   IP_FRAGMENT_MAX_CNT
  #define IP_FRAGMENT_MAX_CNT             0xFF
#endif

//
// Timeout to discard fragment queues. If not all fragments could
// be received until reaching the timeout, timer will discard the queue and the related packets.
//
#ifndef   IP_FRAGMENT_TIMEOUT
  #define IP_FRAGMENT_TIMEOUT             500
#endif
//
// 0: Locks (disables/enables) interrupts for each packet to free from a queue, shorter interrupt disable time.
// 1: Locks (disables/enables) interrupts once for freeing the whole queue, more effective regarding interrupt disable/enable times.
//
#ifndef   IP_FRAGMENT_USE_SINGLE_LOCK
  #define IP_FRAGMENT_USE_SINGLE_LOCK     0
#endif
//
// Switch for the IPv4 base component. Disables IPv4 features.
//
#ifndef   IP_SUPPORT_IPV4
  #define IP_SUPPORT_IPV4                 1
#endif
//
// Check that an address offered via DHCP is really free by sending ARP probes.
// Note: Since emNet version 3.56.1 the default for this define has been reversed,
// it used to be 1 in older versions.
//
#ifndef   IP_DHCPC_CHECK_IP_BEFORE_BOUND
  #define IP_DHCPC_CHECK_IP_BEFORE_BOUND  0
#endif
//
// If enabled the DHCP client will tell the DHCP server to reply via Ethernet unicast instead of broadcast.
// DHCP via broadcast will not just send DISCOVER, but also OFFER, REQUEST and ACK as Ethernet broadcasts
// DHCP via broadcast is essentially obsolete and can cause problems in wireless networks as broadcasts are not automatically retransmitted.
// If you have ancient equipment which can not handle unicast DHCP please set this define to 0 in your IP_Conf.h.
//
#ifndef   DHCPC_USE_UNICAST
  #define DHCPC_USE_UNICAST        1
#endif
...
//
// Delay [ms] between (soft) resetting the PHY and further communication with it.
//
#ifndef   IP_PHY_AFTER_RESET_DELAY
  #define IP_PHY_AFTER_RESET_DELAY        3u
#endif
...
// in the complete generic PHY driver (when using a smart linker).
...
#ifndef   IP_SUPPORT_SO_BINDTODEVICE
  #define IP_SUPPORT_SO_BINDTODEVICE      0
#endif
//
// This feature allows quicker link up detection
// and quicker DHCP negotiation by temporarily increasing
// the link-up timer's period and by triggering the DHCP timer
// routine from the link-up timer routine as soon as the link is up.
//
// The DHCP timer is not modified if a different timeout was
// configured by the user via IP_DHCPC_SetTimeout().
//
#ifndef   IP_SMART_LINK_UP_TIMERS
  #define IP_SMART_LINK_UP_TIMERS         1
#endif
