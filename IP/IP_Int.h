...
**********************************************************************
-------------------------- END-OF-HEADER -----------------------------
Purpose     : Internals used across different layers of the TCP/IP stack
*/
...
//
// Timer period in ms to use when all interfaces are down
// (we want to quickly notice when our link is up)
//
// This value should not be set to less than 100 ms
// because PHY resets can take a long time (e.g. on K66 ~55ms)
//
#if IP_SMART_LINK_UP_TIMERS
#ifndef IP_IFACE_TIMER_LINK_DOWN_PERIOD
  #define IP_IFACE_TIMER_LINK_DOWN_PERIOD 100
#endif

//
// Timer period in ms to use when some interface is up.
//
#ifndef IP_IFACE_TIMER_DEFAULT_PERIOD
  #define IP_IFACE_TIMER_DEFAULT_PERIOD 1000
#endif

IP_TIMER * IP_DHCPC_GetTimerPointer(void);
#endif
...
*       IP_IGMP
*/
typedef struct IP_IGMP_GROUP_STRUCT IP_IGMP_GROUP;
struct IP_IGMP_GROUP_STRUCT {
  IP_IGMP_GROUP* pNext;
  IFACE*         pIFace;
  IP_TIMER       Timer;
  IP_ADDR        GroupIP;
...
typedef enum {
  IP_IGMP_REJOIN_DISABLED = 0,
  IP_IGMP_REJOIN_ENABLED
} IP_IGMP_REJOIN;

typedef struct {
  unsigned (*pfIsInGroup)       (const IFACE* pIFace, IP_ADDR GroupIP);
...
*/
#define IP_IFACE_FLAGS_POLL_FROM_ISR_MASK      (1u << 0)  // Does this interface need polling due to missing interrupt ?
#define IP_IFACE_FLAGS_UPDATE_FILTERS_MASK     (1u << 1)  // Request an update of the MAC filters for this interface.
#define IP_IFACE_FLAGS_NOTIFY_LINK_CHANGE      (1u << 2)  // Pending notification about a change of speed and duplex.
#define IP_IFACE_FLAGS_NOTIFY_HW_LINK_CHANGE   (1u << 3)  // Pending notification about a change of speed, duplex and the HW state.
#define IP_IFACE_FLAGS_NOTIFY_STATE_CHANGE     (1u << 4)  // Pending notification about a change of the HW state.
...
typedef struct {
  ...
  //
  int  (*pfIPv6SendUnicast)   (IP_PACKET * pPacket, const U8 * pDestIP);
  int  (*pfIPv6SendMulticast) (IP_PACKET * pPacket, const U8 * pDestIP);
#endif
#if IP_SUPPORT_STATS_IFACE
...
struct IFACE_STRUCT {
  ...
  IFACE*              pHWIFace;            // Hardware interface used to send packets. Typically NULL for "real" ethernet interfaces; used for PPPoE
  ...
  // Other elements
  //
#if IP_SUPPORT_MULTICAST
  IP_IGMP_GROUP*      pFirstIgmpGroup;
  U32                 IgmpV1ModeTimeout;   // If a timeout timestamp is set, IGMPv2 shall operate in IGMPv1 fallback mode until the timeout expires.
                                           // Each time an IGMPv1 query is received the timeout is renewed.
#endif
...
#if IP_SUPPORT_MULTICAST
  IP_IGMP_REJOIN      IgmpAutoRejoin;      // Auto re-JOIN on link UP.
  U8                  IgmpAlwaysReport;    // Send a REPORT to each IGMP QUERY regardless if a REPORT from another host has been seen.
#endif
  U8                  AdminState;
  U8                  HWState;
  U8                  HasError;
...
int               IP_AddInterface        (const char* sName, const IP_HW_DRIVER* pDriver, IFACE* pHWIFace, unsigned Mtu, unsigned HeaderSize, const IFACE_TYPE* pIFType, U8 MaxProtShiftCnt, U8 TailSize);
...
int      IP_IsIPv6EnabledEx          (unsigned IFaceId);
...
void     IP_RAND_FeedU32             (U32 v);
U32      IP_RAND_GetU32              (void);
...
#define IP_GetRandU32                          IP_RAND_GetU32
...
#define ETH_TYPE_VLAN   htons(0x8100u)  // Ethernet type "VLAN (802.1q)"
#define ETH_TYPE_8021AD htons(0x88A8u)  // Ethernet type "QinQ VLAN (802.1ad)"
...
*
*  Keep the important members of IP_MEM_ALLOC_INFO and IP_MEM_FREE_LIST
*  compatible as they are converted from one to the other.
*  When in need for sizeof(IP_MEM_ALLOC_INFO) or sizeof(IP_MEM_FREE_LIST)
*  the code shall use sizeof(IP_MEM_ALLOC_FREE_UNION) instead as this
*  union will make sure that a size suitable for both is used.
*/
...
#define IP_MEM_SIZEOF_PADDED_STRUCT(s)  ((sizeof(s) + IP_MEM_STRUCT_PADDING - 1u) & ~(IP_MEM_STRUCT_PADDING - 1u))  // Get the sizeof() a struct and pad it to 8-bytes, so further items will be properly aligned in all cases (up to 64-bit).

typedef struct IP_MEM_FREE_LIST_STRUCT IP_MEM_FREE_LIST;
struct IP_MEM_FREE_LIST_STRUCT {
  IP_MEM_FREE_LIST* pNext;
  U32               NumBytes;  // Number of free bytes including the management structure (padded IP_MEM_ALLOC_FREE_UNION).
};
...
int   IP_MEM_GetMemPoolInfo            (IP_MEM_POOL* pPool, IP_MEM_POOL_INFO* pInfo);


//
// Disable the following lint messages for IP allocation macros:
...
//
// Dynamic memory allocation functions return a void pointer that points to the allocated
// memory. The function ensures that the pointer is properly aligned so it can be safely casted
// to any type.
//
//lint --emacro((9087,9079),IP_MEM_ALLOC)
//lint --emacro((9087,9079),IP_MEM_ALLOC_EX)
//lint --emacro((9087,9079),IP_MEM_ALLOC_ZEROED)
//lint --emacro((9087,9079),IP_MEM_ALLOC_ZEROED_EX)
//lint --emacro((9087,9079),IP_MEM_ALLOC_TRANSFER_MEMORY)
//lint --emacro((9087,9079),IP_MEM_ALLOC_TRANSFER_MEMORY_PHYS)
//lint --emacro((9087,9079),IP_MEM_ALLOC_TRANSFER_MEMORY_ZEROED)
#if IP_DEBUG_MEM
...
*       Socket internals
*/
#define SOCKET_WAIT_FLAG_TASK    (1u << 0)  // One or more tasks are waiting on this socket.
#define SOCKET_WAIT_FLAG_SELECT  (1u << 1)  // Socket is used in a select operation.
#define SOCKET_WAIT_FLAG_ABORT   (1u << 2)  // Abort a blocking (read) operation upon waking up and re-checking the socket.
                                            // Set by IP_SOCKET_AbortRead() and checked whenever waking up in wait loops.
                                            // Cleared upon each new cycle of internal wait loop of recv(), recvfrom() and select()
                                            // with the socket being part of the "read" FD_SET .
...
struct SOCKET_STRUCT {
  ...
  U8 volatile                WaitFlags;     // Volatile to make sure that clear->wait_unlock->check_notCleared works.
...
int  IP_IGMP_JoinGroup_NoLock           (unsigned IFaceId, IP_ADDR GroupIP);
int  IP_IGMP_JoinGroup_AutoRejoin_NoLock(unsigned IFaceId, IP_ADDR GroupIP);
...
#if IP_ALLOW_DEINIT
        IP_ON_EXIT_HOOK*     pFirstOnExit;
#endif
        IP_MEM_POOL*         pMemPoolList;
        IP_RX_HOOK*          pfOnRx;
#if IP_SUPPORT_ON_PACKET_TX_CB
        IP_TX_HOOK*          pfOnTx;
#endif
...
U8*      IP_ETH_IgmpIPAddr2HWAddr          (U8* pHWAddr, IP_ADDR IPAddr);
...
//
int  IP_IPV6_UDP_SendAndFree_NoLock(IFACE * pIFace, const IPV6_ADDR * pFHost, U16 FPort, U16 LPort, IP_PACKET * pPacket);
void IP_IPvX_UDP_OnRx              (IP_PACKET * pPacket);   // Rx functions handles IPv4 and IPv6 packets.
...
//
typedef struct ARP_ENTRY_STRUCT ARP_ENTRY;
struct ARP_ENTRY_STRUCT {
  ARP_ENTRY* pNext;
  ARP_ENTRY* pPrev;
  IFACE*     pIFace;
...
typedef int (IP_ARP_ON_CONFLICT_FUNC)(unsigned IFaceId, IP_PACKET* pPacket);

typedef struct {
  IP_ARP_ON_CONFLICT_FUNC* pfHandleAddrConflict;   // Function pointer for ACD.
  IP_ARP_ON_CONFLICT_FUNC* pfCheckAutoIPConflict;  // Function pointer for AutoIP.
...
*/
void IP_DTASK_Init_NoLock(void);
...
*/
#define IP_WIFI_ESS_MASK      (1u << 0)
#define IP_WIFI_IBSS_MASK     (1u << 1)
#define IP_WIFI_PRIVACY_MASK  (1u << 4)

#define IP_WIFI_IE_RSN        0x30u
#define IP_WIFI_IE_VENDOR     0xDDu

//
// IE OUIs
//
#define IP_WIFI_IE_OUI_WPA_VERSION  0x0050F201uL

//
// Unicast cipher OUIs in WPA VERSION IE
//
#define IP_WIFI_IE_OUI_WPA_TKIP  0x0050F202uL
#define IP_WIFI_IE_OUI_WPA_AES   0x0050F204uL
...
void IP_WIFI_OnClientNotification   (unsigned IFaceId, const IP_WIFI_CLIENT_INFO* pInfo, unsigned IsConnected);
...
#if IP_SUPPORT_IPV6
    const IPV6_ADDR * pIPAddrV6;                              // Multicast IPv6 Address pointer.
#endif
  } Addr;
} IP_MDNS_DISPATCHER;

IP_UDP_CONNECTION* IP_MDNS_DispatchOpen (IP_MDNS_DISPATCHER* pHook, U16 Port, unsigned IsIPv6);
void               IP_MDNS_DispatchClose(const IP_MDNS_DISPATCHER* pHook, unsigned IsIPv6);
...
#define IP_NI_CMD_ATTACH                        28   // Attach/allocate driver resources. Called directly when adding an NI driver to the system.
                                                     // The maximum number of interfaces needs to be set before adding any driver, so that drivers
                                                     // can easily allocate resources for up to n interfaces as it has always been done.
#define IP_NI_CMD_ON_SYS_NOTIFY                 29   // Called by the system to notify the driver of a change of the system state.
                                                     // Examples for state notifications that are interesting to the driver would be:
                                                     //   - Getting notified once the configuration phase is done e.g. when we return from
                                                     //     IP_X_Config() . Once configuration is done it is then safe for the driver to
                                                     //     evaluate things like IPv6 being used or not as some hardwae might need to disable
                                                     //     HW checksum in this case.
                                                     //   - Getting notified of changes to the configuration during run-time as necessary.
                                                     //     This could be IPv6 being added late to the system and the NI driver needs to
                                                     //     change its HW checksum handling for this to work or some other protocol with
                                                     //     HW support by the NI needing some configuration to be applied to the hardware.

/*********************************************************************
*
*       IP_STATE_...
*
*       Current state of the stack such as "after config" once IP_Init()
*       returns from IP_X_Config() . Can be used by modules to get
*       notified once it is safe to assume the configuration is safe
*       to be used and
*/
typedef enum {
  IP_STATE_AFTER_CONFIG = 0u   // Can be used to get notified once it is safe to assume the configuration will
                               // not be changed anymore and is safe to be used to make internal auto-configuration
                               // of certain aspects such as being able to use NI driver HW checksum which for
                               // example can not be used with every hardware.
} IP_STATE;
...
*       IP_NOTIFY_EVENT_...
*
*       Main category for stack internal event notification system.
*       Also used for filtering against a single event category.
*/
typedef enum {
  IP_NOTIFY_EVENT_ANY = 0u,  // Only used as filter wildcard when adding hooks. Never used to publish notifications.
  IP_NOTIFY_EVENT_STATE,     // The state the stack is currently in has been updated. Signals "after config" and other IP_STATE_* states.
  IP_NOTIFY_EVENT_CONFIG     // Important parts of the configuration have been changed after IP_X_Config() such as adding IPv6 late or adding a new interface.
} IP_NOTIFY_EVENT;

/*********************************************************************
*
*       IP_NOTIFY_STATE_INFO
*
*  Function description
*    Returns information about a state change of the stack itself.
*/
typedef struct {
  IP_STATE State;  // Current state the stack is in, for example IP_STATE_AFTER_CONFIG .
} IP_NOTIFY_STATE_INFO;

/*********************************************************************
*
*       IP_NOTIFY_CONFIG_ETH_TYPE_INFO
*
*       Information about the Ethernet type added/removed.
*/
typedef struct {
  U16 EthType;  // EthernetType like 0x86DD for IPv6.
} IP_NOTIFY_CONFIG_ETH_TYPE_INFO;

/*********************************************************************
*
*       IP_NOTIFY_CONFIG_IFACE_INFO
*
*       Information about interfaces changed (add/remove).
*/
typedef struct {
  IFACE* pIFace;
} IP_NOTIFY_CONFIG_IFACE_INFO;
...
/*********************************************************************
*
*       IP_NOTIFY_CONFIG_TYPE...
*
*       Type of configuration change that happened.
*/
typedef enum {
  IP_NOTIFY_CONFIG_TYPE_ADD_ETH_TYPE = 0u,  // An Ethernet type (e.g. 0x86DD for IPv6) has been added.
  IP_NOTIFY_CONFIG_TYPE_REMOVE_ETH_TYPE,    // An Ethernet type (e.g. 0x86DD for IPv6) has been removed.
  IP_NOTIFY_CONFIG_TYPE_ADD_IFACE,          // An interface has been added.
  IP_NOTIFY_CONFIG_TYPE_REMOVE_IFACE        // An interface has been removed.
} IP_NOTIFY_CONFIG_TYPE;

/*********************************************************************
*
*       IP_NOTIFY_CONFIG_INFO
*
*  Function description
*    Returns information about a configuration change in the stack.
*/
typedef struct {
  IP_NOTIFY_CONFIG_TYPE            ConfigType;
  union {
    IP_NOTIFY_CONFIG_ETH_TYPE_INFO EthType;
    IP_NOTIFY_CONFIG_IFACE_INFO    IFace;
  } Data;
} IP_NOTIFY_CONFIG_INFO;

/*********************************************************************
*
*       IP_NOTIFY_EVENT_INFO
*
*  Function description
*    Returns information about events that occur in the stack.
*    The info callback parameter \c{Event} has to be evaluated for
*    further information if there is more info about the event and
*    what part of the union is the information to look at.
*/
typedef struct {
  union {
    IP_NOTIFY_STATE_INFO  State;
    IP_NOTIFY_CONFIG_INFO Config;
  } Data;
} IP_NOTIFY_INFO;

/*********************************************************************
*
*       IP_NOTIFY_ON_EVENT_FUNC
*
*  Function description
*    Callback executed for event notifications from the stack.
*
*  Parameters
*    Event: Type of event and what part of the \c{pInfo->Data} union
*           to look at for more information.
*    pInfo: Further information of type IP_NOTIFY_INFO about the
*           actual information available.
*/
typedef void (IP_NOTIFY_ON_EVENT_FUNC)(IP_NOTIFY_EVENT Event, IP_NOTIFY_INFO* pInfo);

typedef struct IP_HOOK_ON_NOTIFY_STRUCT IP_HOOK_ON_NOTIFY;
struct IP_HOOK_ON_NOTIFY_STRUCT {
  IP_HOOK_ON_NOTIFY*       pNext;
  IP_NOTIFY_ON_EVENT_FUNC* pf;
  IP_NOTIFY_EVENT          Filter;
};

void IP_NOTIFY_AddHook(IP_HOOK_ON_NOTIFY* pHook, IP_NOTIFY_ON_EVENT_FUNC* pf, IP_NOTIFY_EVENT Filter);
void IP_NOTIFY_RemoveHook(IP_HOOK_ON_NOTIFY* pHook);
void IP_NOTIFY_Signal (IP_NOTIFY_EVENT Event, IP_NOTIFY_INFO* pInfo);
...
int                IP_PHY_GENERIC_ReadReg      (unsigned IFaceId, IP_PHY_CONTEXT_EX* pContextEx, unsigned RegIndex, unsigned* pData);
int                IP_PHY_GENERIC_WriteReg     (unsigned IFaceId, IP_PHY_CONTEXT_EX* pContextEx, unsigned RegIndex, unsigned Data);
...
typedef struct {  // Values needed for all calculations during Sync, Delay, Offset, Drift, etc.
  ...
  U64 T3DelayTxSec;
  U64 T4DelayRxSec;
  U64 FrtSec;   // Seconds part of the Forward Residence Time (SYNC delay in TC).
  U64 RrtSec;   // Seconds part of the Reverse Residence Time (DELAY-REQ delay in TC).
  U32 T1SyncTxNSec;
  U32 T2SyncRxNSec;
  U32 T3DelayTxNSec;
  U32 T4DelayRxNSec;
  U32 FrtNSec;  // Nanoseconds part of the Forward Residence Time (SYNC delay in TC).
  U32 RrtNSec;  // Nanoseconds part of the Reverse Residence Time (DELAY-REQ delay in TC).
  //
...
struct IP_PTP_INT_CONTEXT_STRUCT {                                            // NOLINT(clang-analyzer-optin.performance.Padding) (Public struct) Forward declared in IP.h .
  ...
  const IP_PTP_OC_LOGIC_API*     apfLogicFallback[IP_PTP_ENDPOINT_TYPE_MAX];  // Fallback (suggestion) for an OC handler if the driver does not come with its own master handler to install.
        ...
        // Others.
        //
        IP_PTP_PROT_TYPE         Prot;  // Set early in _PTP_OnRx() and used by by sub-calls processing the same request.
...
IP_PACKET*          IP_PTP_AllocPacket        (unsigned NumBytes, IP_PTP_PROT_TYPE Prot);
...
void                IP_PTP_SendPacket         (IP_PACKET* pPacket, IP_PTP_PROT_TYPE Prot);
...
typedef struct {
  ...
  U8  PacketDataShiftCnt;    // ShiftCnt for ideal packet data alignment. Could avoid a memcpy into an extra buffer that is aligned.
                             ...
                             // number of bytes is prepend to IP_PACKET.pBuffer that can be used to store SPI overhead without
